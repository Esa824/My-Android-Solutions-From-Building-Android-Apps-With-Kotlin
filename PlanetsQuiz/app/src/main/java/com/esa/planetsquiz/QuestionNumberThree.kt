...
class QuestionNumberThree : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.question).text = getString(R.string.question_number_three)
    ...
    private fun setupPlanetButtonClickListener(buttonId: Int, intent: Intent) {
        findViewById<MaterialTextView>(buttonId).setOnClickListener {
            if (buttonId == R.id.uranus) {
                findViewById<TextView>(R.id.answer).text = "CORRECT!".plus(" ").plus(getString(R.string.answer_question_number_three))
                setResult(Activity.RESULT_OK, intent)
            } else {
                findViewById<TextView>(R.id.answer).text = "WRONG!".plus(" ").plus(getString(R.string.answer_question_number_three))
